...
static void tre_print_reach(const tre_tnfa_t *tnfa, tre_tnfa_approx_reach_t *reach, int pos,
                            int num_tags)
{
    int id;
...
/* Sets the matching parameters in `reach' to the ones defined in the `pa'
   ...
   `default_params'. */
inline static void tre_set_params(tre_tnfa_approx_reach_t *reach, int *pa,
...
reg_errcode_t tre_tnfa_run_approx(const tre_tnfa_t *tnfa, const void *string, ssize_t len,
                                  ...
                                  regaparams_t default_params, int eflags, int *match_end_ofs)
{
    ...
    DPRINT(("tre_tnfa_run_approx, input type %d, len %zd, eflags %d, "
            "match_tags %p\n",
            type, len, eflags, match_tags));
    DPRINT(("max cost %d, ins %d, del %d, subst %d\n", default_params.max_cost,
            default_params.cost_ins, default_params.cost_del, default_params.cost_subst));
    ...
    while (/*CONSTCOND*/ (void)1, 1)
    {
            ...
            for (trans = tnfa->initial; trans->state; trans++)
            {
               ...
               there. */
                if (reach_next[stateid].pos < pos)
                {
                    ...
                    if (trans->params)
                    {
                        tre_set_params(&reach_next[stateid], trans->params, default_params);
                    }
       ...
       give a better match than what we already have. */
        for (id = 0; id < tnfa->num_states; id++)
        {
            ...
            if (reach[id].costs[depth][TRE_M_NUM_INS] + 1 > reach[id].params.max_ins)
            {
                continue; /* Too many inserts. */
            }

            /* Check total number of errors at current depth. */
            if (reach[id].costs[depth][TRE_M_NUM_ERR] + 1 > reach[id].params.max_err)
            {
            ...
            DPRINT(
                ("  insert: from %d to %d, cost %d: ", id, id, reach[id].costs[depth][TRE_M_COST]));
            if (reach_next[id].pos == pos && (cost0 >= reach_next[id].costs[0][TRE_M_COST]))
            {
                DPRINT(("lose\n"));
                continue;
            }
       ...
       no more states can be reached with better costs. */
        {
            ...
            while (deque_end != deque_start)
            {
                ...
                for (trans = reach_p->state; trans->state; trans++)
                {
                    ...
                    DPRINT(("  delete: from %03d to %03d, cost %d (%d): ", id, dest_id, cost0,
                            reach_p->params.max_cost));
                    ...
                    if (reach_next[dest_id].pos == pos &&
                        (cost0 > reach_next[dest_id].costs[0][TRE_M_COST] ||
                         (cost0 == reach_next[dest_id].costs[0][TRE_M_COST] &&
                          (!match_tags || !tre_tag_order(num_tags, tnfa->tag_directions, tmp_tags,
                    ...
                    if (trans->params)
                    {
                        tre_set_params(&reach_next[dest_id], trans->params, default_params);
                    }
                    ...
                    if (trans->state == tnfa->final &&
                        (match_eo < 0 || match_costs[TRE_M_COST] > cost0 ||
        ...
        for (id = 0; id < tnfa->num_states; id++)
        {
            ...
            for (trans = reach[id].state; trans->state; trans++)
            {
                ...
                if (trans->code_min > (tre_cint_t)prev_c || trans->code_max < (tre_cint_t)prev_c)
                {
                    ...
                    if (reach[id].costs[depth][TRE_M_NUM_SUBST] + 1 > reach[id].params.max_subst)
                    {
                    ...
                    DPRINT(("  subst,  from %03d to %03d, cost %d: ", id, dest_id, cost0));
                }
                else
                {
                    DPRINT(("  exact,  from %03d to %03d, cost %d: ", id, dest_id, cost0));
                }
                ...
                if (reach_next[dest_id].pos == pos &&
                    (cost0 > reach_next[dest_id].costs[0][TRE_M_COST] ||
                     (cost0 == reach_next[dest_id].costs[0][TRE_M_COST] &&
                      !tre_tag_order(num_tags, tnfa->tag_directions, tmp_tags,
                                     reach_next[dest_id].tags))))
                {
                    DPRINT(("lose\n"));
                    continue;
                ...
                if (trans->state == tnfa->final &&
                    (match_eo < 0 || cost0 < match_costs[TRE_M_COST] ||
                     (cost0 == match_costs[TRE_M_COST] && num_tags > 0 &&
