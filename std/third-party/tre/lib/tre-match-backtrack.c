...
#define xafree(obj)                                                                                \
    do                                                                                             \
    {                                                                                              \
    } while (0) /* do nothing, obj was obtained with alloca() */
#else /* !TRE_USE_ALLOCA */
...
retry:
{
    ...
    for (trans_i = tnfa->initial; trans_i->state; trans_i++)
    {
        ...
        else
        {
            ...
            BT_STACK_PUSH(pos, str_byte, str_wide, trans_i->state, trans_i->state_id, next_c, tags,
                          mbstate);
            {
    ...
    while (/*CONSTCOND*/ (void)1, 1)
    {
...
#ifdef TRE_DEBUG
        DPRINT(("%3zd:%2lc/%05d | %p ", pos, (tre_cint_t)next_c, (int)next_c, state));
        {
            int i;
            for (i = 0; i < tnfa->num_tags; i++)
            {
        ...
        if (trans_i->state && trans_i->assertions & ASSERT_BACKREF)
        {
            ...
            if (len < 0)
            {
                if (type == STR_USER)
                {
                    result = str_source->compare((unsigned)so, (unsigned)pos, (unsigned)bt_len,
                                                 str_source->context);
                }
#ifdef TRE_WCHAR
                else if (type == STR_WIDE)
                {
                    result = wcsncmp((const wchar_t *)string + so, str_wide - 1, (size_t)bt_len);
                }
#endif /* TRE_WCHAR */
                else
                {
        ...
        for (trans_i = state; trans_i->state; trans_i++)
        {
            DPRINT(("  transition %d-%d (%c-%c) %d to %d\n", trans_i->code_min, trans_i->code_max,
                    trans_i->code_min, trans_i->code_max, trans_i->assertions, trans_i->state_id));
            if (trans_i->code_min <= (tre_cint_t)prev_c && trans_i->code_max >= (tre_cint_t)prev_c)
            {
                ...
                else
                {
                    ...
                    DPRINT(("  saving state %d for backtracking\n", trans_i->state_id));
                    BT_STACK_PUSH(pos, str_byte, str_wide, trans_i->state, trans_i->state_id,
                                  next_c, tags, mbstate);
                    {
                        int *tmp;
