...
tre_ast_node_t *tre_ast_new_node(tre_mem_t mem, tre_ast_type_t type, size_t size)
{
    ...
    node = (decltype(node))tre_mem_calloc(mem, sizeof(*node));
    if (!node)
    {
        return NULL;
    }
    node->obj = (decltype(node->obj))tre_mem_calloc(mem, size);
    if (!node->obj)
    {
        return NULL;
    }
...
static void tre_findent(FILE *stream, int i)
{
    while (i-- > 0)
    {
        fputc(' ', stream);
...
static void tre_do_print(FILE *stream, tre_ast_node_t *ast, int indent)
{
    ...
    case LITERAL:
        ...
        else
        {
            fprintf(stream,
                    ...
                    "%d tags\n",
                    code_min, code_max, code_min, code_max, pos, ast->submatch_id, num_tags);
        }
        break;
    case ITERATION:
        iter = ast->obj;
        fprintf(stream, "iteration {%d, %d}, sub %d, %d tags, %s\n", iter->min, iter->max,
                ast->submatch_id, num_tags, iter->minimal ? "minimal" : "greedy");
...
static void tre_ast_fprint(FILE *stream, tre_ast_node_t *ast)
{
